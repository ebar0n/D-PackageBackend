...
class UserAccountViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def forgot_password(self, request, *args, **kwargs):
        ...
        if user.token_expires and user.token_expires > timezone.now():
            return Response({
                'status': 'Bad request',
                'message': 'We already sent a link to reset the password.'
        ...
        send_mail.run([user.email], 'Reset password', 'reset_password.html', data)
    ...
    def reset_password(self, request, *args, **kwargs):
        ...
        if user.token_expires and user.token_expires < timezone.now():
            return Response({
                'status': 'Bad request',
                'message': 'The token has already expired, please request a new one.'
            }, status.HTTP_400_BAD_REQUEST)
